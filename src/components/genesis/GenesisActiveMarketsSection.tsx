...
export type GenesisActiveMarketsSectionProps = {
  ...
  marksResults:
    | NonNullable<
        ReturnType<
          typeof import("@/hooks/useGenesisPageData").useGenesisPageData
        >["marksResults"]
      >
    | undefined;
...
export function GenesisActiveMarketsSection(
  props: GenesisActiveMarketsSectionProps,
) {
  const {
    showHeaders,
    genesisViewBasic,
            ...
            label={
              <div>
                ...
                <div className="text-xs opacity-90 mb-2">
                  Zapper-supported assets are zapped in with no slippage. Any
                  other ERC20s are swapped with Velora.
                </div>
  ...
  const marketRows = displayedActiveMarkets.map(([id, mkt]) => {
    ...
    const genesisGreyRow = genesisPreviewSoon || genesisClaimOnlyCfg;
    ...
    const displayMarketName =
      formatGenesisMarketDisplayName(rawDisplayMarketName);
    ...
    const totalDeposits = readContractRowResult<bigint>(totalDepositsReads, mi);
    ...
    const underlyingSymbol =
      (mkt as GenesisMarketConfig).collateral?.underlyingSymbol ||
      collateralSymbol; // The underlying/base token
    ...
    const { collateralPriceUSD, priceError } = computeGenesisRowUsdPricing({
      underlyingSymbol,
    ...
    const showTestMarketTag = (mkt as GenesisMarketConfig).test === true;
    const depositCapData = getGenesisDepositCapData({
      ...
      genesisTokenCapAmount: (mkt as GenesisMarketConfig).genesisTokenCapAmount,
    });
    ...
    if (genesisViewBasic) {
      return (
        <GenesisCompactMarketCard
          ...
          chainLogo={
            (mkt as GenesisMarketConfig).chain?.logo || "icons/eth.png"
          }
        ...
        >
              ...
              {!isEnded && (
                <div className="hidden md:block">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    />
                    <div
                      className={`flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide whitespace-nowrap ${isCollateralOnlyRow ? "bg-[#1E4775]/10 text-[#1E4775]" : "bg-blue-100 text-blue-700"}`}
                    >
                      {!isCollateralOnlyRow && (
                        <ArrowPathIcon className="w-2.5 h-2.5" />
                      )}
                      <span>
                        {isCollateralOnlyRow ? "Collateral" : "Any Token"}
                      </span>
                    </div>
                  </div>
                </div>
                      ...
                      <div>
                        {showTestMarketTag ? (
                          <SimpleTooltip
                            ...
                            side="top"
                          >
                            <span className={GENESIS_MARKET_TEST_TAG_CLASS}>
                              TEST
                            </span>
                          </SimpleTooltip>
                        ) : isProcessing ? (
                    ...
                    >
                      <div className="flex items-center justify-center gap-1 cursor-help">
                        ...
                        <Image
                          src={getLogoPath(
                            rowPeggedSymbol ||
                              (mkt as GenesisMarketConfig).peggedToken
                                ?.symbol ||
                              "haPB",
                          )}
                          alt={
                    ...
                    >
                      <div className="flex items-center justify-center gap-1 cursor-help">
                        ...
                        <Image
                          src={getLogoPath(
                            rowLeveragedSymbol ||
                              (mkt as GenesisMarketConfig).leveragedToken
                                ?.symbol ||
                              "hsPB",
                          )}
                          alt={
          ...
          >
            ...
            <div className="min-w-0 overflow-hidden">
              <div className="flex items-center justify-center gap-1.5 flex-wrap">
                ...
                <span className="text-[#1E4775]/60 hidden xl:inline">:</span>
            ...
            {!isEnded ? (
              ...
              >
                  ...
                  label={
                    isCollateralOnlyRow ? (
                      <div>
                        <div className="font-semibold mb-1">
                          {isMegaEthRow
                            ? "Collateral only (MegaETH)"
                            : "Collateral only"}
                        </div>
                        <div className="text-xs opacity-90">
                          Deposit accepted collateral assets only. No token swap
                          on this chain.
                        </div>
                      </div>
                    ) : (
                      <div>
                        ...
                        <div className="text-xs opacity-90 mb-2">
                          Zapper-supported assets are zapped in with no
                          slippage. Any other ERC20s are swapped with Velora.
                        </div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[10px] opacity-75">
                            Zapper-supported:
                ...
                >
                  <div
                    className={`flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide cursor-help whitespace-nowrap ${isCollateralOnlyRow ? "bg-[#1E4775]/10 text-[#1E4775]" : "bg-blue-100 text-blue-700"}`}
                  >
                    {!isCollateralOnlyRow && (
              ...
              <div className="text-center min-w-0">
                {showTestMarketTag ? (
                  <SimpleTooltip
                    ...
                    side="top"
                  >
                    <span className={GENESIS_MARKET_TEST_TAG_CLASS}>TEST</span>
                  </SimpleTooltip>
                ) : isProcessing ? (
                  <SimpleTooltip
                    label={
                      <div className="text-left max-w-xs">
                        ...
                        <div className="text-xs space-y-1">
                          <p>
                            The Harbor team will transfer collateral and make
                            anchor + sail tokens claimable imminently.
                          </p>
                          <p className="mt-2">
                            <strong>Deposits:</strong> Still possible until
                            claiming opens. Complete your deposit before the
                            processing ends.
                          </p>
                          <p>
                            <strong>Marks:</strong> Still being earned during
                            processing. Bonus marks will be applied at the end
                            of processing.
                          </p>
                        </div>
                      </div>
            ...
            >
              <div className="flex items-center justify-center">
                <GenesisMarketRowClaimActions
                  ...
                  isClaimingThisMarket={claimingMarket === id}
                  manageDisabled={
                    !genesisAddress || genesisDepositWithdrawBlocked
                  }
                  onClaim={() =>
                    claimMarket({
                      marketId: id,
          ...
          {!isEnded &&
            ...
            (() => {
              ...
              const capCurrent = useTokenCap ? totalDepositsAmount : cumulative;
              const progressPct =
                capTotal > 0 ? Math.min(100, (capCurrent / capTotal) * 100) : 0;
              const yieldRevSharePct = maidenVoyageYieldOwnerSharePercent(
              ...
              return (
                <div className="px-2 pt-2.5 pb-0.5 border-t border-[#1E4775]/10 -mb-1 mt-1">
                    ...
                    <SimpleTooltip
                      label={
                        <span className="text-xs leading-relaxed block max-w-xs">
                          Deposits count toward this market&apos;s USD cap. At
                          genesis close, your counted deposit ÷ cap fixes your{" "}
                          <strong>ownership share</strong> of this market&apos;s
                          maiden voyage yield pool. Voyage boost is separate and
                          only adjusts weight after genesis. Only a configured
                          share of fee + carry revenue is credited to the maiden
                          pool (see expanded row).
                        </span>
                      }
                    >
                    ...
                    <p className="text-[10px] text-[#1E4775]/80 leading-snug mb-0.5">
                      ...
                      (!useTokenCap && capRow?.capFilled) ? (
                        ...
                        <>
                          ...
                          <span className="font-semibold">ownership</span> is
                          still open (headroom for new deposits).
                          {yieldRevSharePct != null ? (
                            <>
                              {" "}
                              {yieldRevSharePct}% of attributed mint/redeem fee
                              + collateral carry revenue is credited to this
                              pool; that slice is split among owners.
                            </>
                          ) : (
                            <>
                              {" "}
                              A configured share of attributed mint/redeem fee +
                              collateral carry revenue is credited to this pool;
                              that slice is split among owners.
                            </>
                          )}{" "}
                  ...
                  {(ownership > 0 || (counted > 0 && ownership === 0)) && (
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-[#1E4775]/80">
                      ...
                      {counted > 0 && ownership === 0 && (
                        <span>Counted toward cap: {formatUSD(counted)}</span>
                      )}
                    </div>
                  )}
                </div>
