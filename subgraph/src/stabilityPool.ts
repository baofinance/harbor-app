...
import { updateStabilityPoolMarksInTotal } from "./marksAggregation";
import {
...
function getStabilityPoolMultiplier(
  ...
  const sourceType =
    poolType === "collateral" ? "stabilityPoolCollateral" : "stabilityPoolSail";
...
function queryPoolDepositBalance(
  poolAddress: Address,
  userAddress: Address
): BigInt {
  const pool = StabilityPool.bind(poolAddress);
...
function accumulateMarks(
  ...
  const marksAccumulated = deposit.balanceUSD
    .times(marksPerDollarPerDay)
...
export function handleStabilityPoolDeposit(
  ...
  const actualBalance = queryPoolDepositBalance(
  ...
  const peggedTokenAddress = getPeggedTokenAddress(
    Address.fromBytes(poolAddress)
  );
  if (peggedTokenAddress != null) {
    deposit.balanceUSD = calculateDepositUSD(
      actualBalance,
      peggedTokenAddress,
      event.block
    );
  ...
  if (
    deposit.firstDepositAt.equals(BigInt.fromI32(0)) &&
    actualBalance.gt(BigInt.fromI32(0))
  ) {
...
export function handleStabilityPoolWithdraw(
  event: StabilityPoolWithdrawEvent
): void {
  const poolAddress = event.address;
  const userAddress = event.params.user;
  const timestamp = event.block.timestamp;
...
export function handleStabilityPoolDepositChange(
  event: UserDepositChangeEvent
): void {
  const poolAddress = event.address;
  const userAddress = event.params.user;
  ...
  if (
    deposit.firstDepositAt.equals(BigInt.fromI32(0)) &&
    newBalance.gt(BigInt.fromI32(0))
  ) {
    deposit.firstDepositAt = timestamp;
  }
