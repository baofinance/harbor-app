...
**UI− / UI+:** `/anchor` is in [`PAGE_LAYOUT_INDEX_EXACT_PATHS`](../../config/pageLayoutToggleRoutes.ts) with **`usePageLayoutPreference`** on the page (Basic = title + toolbar + list; Extended = hero cards, stats strip, subgraph banner, rewards bar). See **[`docs/INDEX_PAGE_PATTERN.md`](../../docs/INDEX_PAGE_PATTERN.md)**.

---

## Phases 0–3 status (Anchor index refactor)

Full portable checklist lives in **[`docs/INDEX_PAGE_REFACTOR_PLAYBOOK.md`](../../docs/INDEX_PAGE_REFACTOR_PLAYBOOK.md)**. Sail’s finished shape is **[`src/app/sail/README.md`](../sail/README.md)**. Use the table below so Phases 0–3 line up with what is **done**, **partial**, or **open** in this repo.
...
| 3 | Duplicate helpers → `utils/anchor*` or `anchorDisplayFormat.ts` | **Partial:** [`utils/anchor.ts`](../../utils/anchor.ts) + [`utils/anchorDisplayFormat.ts`](../../utils/anchorDisplayFormat.ts) for shared formatters; table/grid still also import `@/utils/anchor` where historical. |
...
| # | Item | Status |
|---|------|--------|
| 1 | Stable `React.memo` market row + stable callbacks | **Partial:** [`AnchorMarketGroupCollapsedRow`](../../components/anchor/AnchorMarketGroupCollapsedRow.tsx) wraps the **collapsed** grouped row; `onToggleExpand` / `onOpenManage` are wired from `page.tsx`. |
| 2 | Expanded group panel extraction | **Done:** [`AnchorMarketGroupExpandedSection`](../../components/anchor/AnchorMarketGroupExpandedSection.tsx) holds the **group expanded** UI (positions, withdrawal rows, per-market stats). [`AnchorMarketExpandedView`](../../components/anchor/AnchorMarketExpandedView.tsx) remains a separate, unused building block — reconcile or remove later if redundant. |
| 3 | Toolbar + `*MarketsSections` parity (Genesis/Sail) | **Partial:** [`AnchorMarketsToolbar`](../../components/anchor/AnchorMarketsToolbar.tsx) + [`AnchorMarketsSections`](../../components/anchor/AnchorMarketsSections.tsx) wrap the stability-pool toolbar + list. |
| 4 | `next/dynamic` for heavy charts | **N/A today** — Anchor index has no `PriceChart`/`recharts` in `page.tsx`; Sail uses dynamic chart in [`SailMarketExpandedView`](../sail/SailMarketExpandedView.tsx). Add when/if charts ship here. |
...
| [`useAnchorPageData`](../../hooks/anchor/useAnchorPageData.ts) | **Phase 2 (partial):** composes reads + derived lists (`anchorMarkets`, prices, marks, rewards, positions, `allMarketsData`, **`anchorStats`**, **`claimAllPositions`**, etc.); `page.tsx` still owns expanded rows + modals. |
| [`AnchorMarketGroupExpandedSection`](../../components/anchor/AnchorMarketGroupExpandedSection.tsx) | **Group** expanded panel (wired from `page.tsx`). [`AnchorMarketExpandedView`](../../components/anchor/AnchorMarketExpandedView.tsx) is a separate file, not wired — optional follow-up. |
...
- [`src/utils/anchorDisplayFormat.ts`](../../utils/anchorDisplayFormat.ts) — display helpers where split from core `anchor.ts`.
...
Header ([`AnchorMarketsTableHeader`](../../components/anchor/AnchorMarketsTableHeader.tsx)), collapsed group rows ([`AnchorMarketGroupCollapsedRow`](../../components/anchor/AnchorMarketGroupCollapsedRow.tsx)), and wallet-summary rows share templates from [`anchorMarketsTableGrid.ts`](../../components/anchor/anchorMarketsTableGrid.ts): e.g. **`ANCHOR_MARKETS_TABLE_ROW_LG_CLASSNAME`**, **`ANCHOR_MARKETS_TABLE_ROW_MD_CLASSNAME`**, **`ANCHOR_MARKETS_WALLET_ROW_*`**. Keep header + body class strings in sync when changing column counts.

---

## Modals (high level)

- **[`AnchorDepositWithdrawModal`](../../components/AnchorDepositWithdrawModal.tsx)** — primary deposit/withdraw surface: **`DefinedMarket`** on `market` / `allMarkets`; optional contract keys (e.g. `peggedTokenZap`, `collateralPrice`) via **`anchorAddressByName()`** from [`types/anchor.ts`](../../types/anchor.ts) (same idea as Sail’s `addressByName` on `SailManageModal`). `initialTab` accepts legacy names (`mint`, `redeem`, `deposit-mint`, `withdraw-redeem`) mapped internally to `deposit` / `withdraw`.
...
- Index tuples + batched reads: [`src/types/anchor.ts`](../../types/anchor.ts) — `AnchorMarketTuple`, `AnchorContractReads`; used by [`useAnchorContractReads`](../../hooks/anchor/useAnchorContractReads.ts), [`useAnchorMarketData`](../../hooks/anchor/useAnchorMarketData.ts) (`MarketData.market`), [`useGroupedMarkets`](../../hooks/anchor/useGroupedMarkets.ts), etc.
...
4. **UI− / UI+** — **Done:** `/anchor` in [`PAGE_LAYOUT_INDEX_EXACT_PATHS`](../../config/pageLayoutToggleRoutes.ts); [`page.tsx`](page.tsx) uses [`usePageLayoutPreference`](../../contexts/PageLayoutPreferenceContext.tsx) like Genesis/Sail.
...
1. **Phase 4 typing** — [`src/types/anchor.ts`](../../types/anchor.ts): `AnchorMarketTuple`, `AnchorContractReads`, `anchorAddressByName`; hook signatures (`useAnchorContractReads`, `useAnchorMarketData`, `useGroupedMarkets`, rewards/marks/prices/deposits/token metadata, transactions) and [`calculateReadOffset`](../../utils/anchor/calculateReadOffset.ts) updated; **`MarketData.market`** and **`GroupedMarket`** use **`DefinedMarket`**.
...
3. **Table / row props** — [`AnchorMarketGroupCollapsedRow`](../../components/anchor/AnchorMarketGroupCollapsedRow.tsx) + [`page.tsx`](page.tsx) manage modal state aligned with **`DefinedMarket`**.
